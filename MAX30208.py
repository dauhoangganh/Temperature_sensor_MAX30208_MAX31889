import machine
import utime
from ustruct import unpack, unpack_from, pack
import sys
...
ALARM_LOW = 0x12 #ALARM_LOW_MSB register
ALARM_HIGH = 0x10 #ALARM_HIGH_MSB register
...
class MAX30208:
    def __init__(
        ...
        self.i2c=i2c
        id = self.i2c.readfrom_mem(MAX30208_ADDR, PART_IDENTIFIER, 1)
        if (id != bytearray((DEVICE_ID,))):
            print("\nERROR: Could not communicate with MAX30208")
            sys.exit()
        else:
            print("\nSuccessfully connected to MAX30208")
        ...
        #set low threshold
        self.low_threshold = int(low_thre/0.005) 
        self.i2c.writeto_mem(MAX30208_ADDR, ALARM_LOW, pack(">h", self.low_threshold)) 

        #set high threshold
        self.high_threshold = int(high_thre/0.005) 
        self.i2c.writeto_mem(MAX30208_ADDR, ALARM_HIGH, pack(">h", self.high_threshold)) 

    def measure_temperature(self):  
        #set convert_T bit of temp_sensor_setup register to 1 to start temperature measurement
        temp_sensor_field = self.i2c.readfrom_mem(MAX30208_ADDR, TEMP_SENSOR_SETUP, 1)
        temp_sensor_field = unpack(">b", temp_sensor_field)[0]
        temp_sensor_field = set_bit(temp_sensor_field, 0)
        self.i2c.writeto_mem(MAX30208_ADDR, TEMP_SENSOR_SETUP, bytearray([temp_sensor_field]))
        utime.sleep_ms(100)
        ...
        status = self.i2c.readfrom_mem(MAX30208_ADDR, STATUS, 1)
        status = unpack(">b", status)[0]
        
        # Check FIFO STATUS
        if get_bit(status, A_FULL_BIT):
            overflow_counter = self.i2c.readfrom_mem(MAX30208_ADDR, FIFO_OF_CTR_REG, 1)
            ...
            #Flush FIFO
            FIFO_Config2 =self.i2c.readfrom_mem(MAX30208_ADDR, FIFO_CONF2_REG, 1)
        ...
        if get_bit(status, MAX_TEMP_RDY_BIT): 
            temperature_data =self.i2c.readfrom_mem(MAX30208_ADDR, FIFO_DATA, 2)
            temperature_data = unpack(">h", temperature_data)[0]
            print("\nTemperature in deg C: ", self.convert_to_deg_C(temperature_data))
            if get_bit(status, 2):
                print("\nALERT: Low threshold reached")
                
            if get_bit(status, 1):
                print("\nALERT: High threshold reached")
            # print("Num_available samples in FIFO after last read: ", self.No_of_samples_FIFO())
        utime.sleep(1)

    ########## SEt GPIO mode function #################################################
    def set_GPIO_mode(self, GPIO0_mode, GPIO1_mode):
        GPIO_reg_value = self.i2c.readfrom_mem(MAX30208_ADDR, GPIO_SETUP_REG, 1)
        ...
        self.i2c.writeto_mem(MAX30208_ADDR, GPIO_SETUP_REG, bytearray([GPIO_reg_value]))
    ####################################################################################
    ...
    def No_of_samples_FIFO(self):
        ovf_counter = self.i2c.readfrom(MAX30208_ADDR, FIFO_OF_CTR_REG, 1)
        data_counter = self.i2c.readfrom(MAX30208_ADDR, FIFO_DATA_COUNTER_REG, 1)
